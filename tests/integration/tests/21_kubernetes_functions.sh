...
create_template "k8s_resource_strings.tmpltool" '{{ k8s_resource_request(cpu="500m", memory="512Mi") }}'
OUTPUT=$(run_binary "k8s_resource_strings.tmpltool")
...
create_template "k8s_resource_numeric_cpu.tmpltool" '{{ k8s_resource_request(cpu=0.5, memory="512Mi") }}'
OUTPUT=$(run_binary "k8s_resource_numeric_cpu.tmpltool")
assert_contains "$OUTPUT" 'cpu: "500m"' "numeric CPU converted to millicores"

# Test 3: k8s_resource_request - whole number CPU
create_template "k8s_resource_whole_cpu.tmpltool" '{{ k8s_resource_request(cpu=2, memory="1Gi") }}'
OUTPUT=$(run_binary "k8s_resource_whole_cpu.tmpltool")
assert_contains "$OUTPUT" 'cpu: "2000m"' "whole number CPU converted to millicores"

# Test 4: k8s_resource_request - numeric memory (Mi)
create_template "k8s_resource_numeric_mem_mi.tmpltool" '{{ k8s_resource_request(cpu="500m", memory=512) }}'
OUTPUT=$(run_binary "k8s_resource_numeric_mem_mi.tmpltool")
assert_contains "$OUTPUT" 'memory: "512Mi"' "numeric memory converted to Mi"

# Test 5: k8s_resource_request - numeric memory (Gi)
create_template "k8s_resource_numeric_mem_gi.tmpltool" '{{ k8s_resource_request(cpu="1000m", memory=1024) }}'
OUTPUT=$(run_binary "k8s_resource_numeric_mem_gi.tmpltool")
assert_contains "$OUTPUT" 'memory: "1Gi"' "numeric memory converted to Gi"

# Test 6: k8s_resource_request - both numeric
create_template "k8s_resource_both_numeric.tmpltool" '{{ k8s_resource_request(cpu=1.5, memory=2048) }}'
OUTPUT=$(run_binary "k8s_resource_both_numeric.tmpltool")
...
create_template "k8s_deployment.tmpltool" 'apiVersion: apps/v1
kind: Deployment
metadata:
  name: my-app
spec:
...
OUTPUT=$(run_binary "k8s_deployment.tmpltool")
...
create_template "k8s_resource_vars.tmpltool" '{% set app_config = {"cpu": "250m", "memory": "256Mi"} %}
resources:
  {{ k8s_resource_request(cpu=app_config.cpu, memory=app_config.memory) | indent(2) }}'
OUTPUT=$(run_binary "k8s_resource_vars.tmpltool")
...
create_template "k8s_resource_env.tmpltool" '{% set env = "production" %}
...
OUTPUT=$(run_binary "k8s_resource_env.tmpltool")
...
create_template "k8s_label_simple.tmpltool" '{{ k8s_label_safe(value="my-app") }}'
OUTPUT=$(run_binary "k8s_label_simple.tmpltool")
assert_equals "my-app" "$OUTPUT" "simple label unchanged"

# Test 11: k8s_label_safe - uppercase
create_template "k8s_label_uppercase.tmpltool" '{{ k8s_label_safe(value="MyApp") }}'
OUTPUT=$(run_binary "k8s_label_uppercase.tmpltool")
assert_equals "myapp" "$OUTPUT" "uppercase converted to lowercase"

# Test 12: k8s_label_safe - spaces
create_template "k8s_label_spaces.tmpltool" '{{ k8s_label_safe(value="My App Name") }}'
OUTPUT=$(run_binary "k8s_label_spaces.tmpltool")
assert_equals "my-app-name" "$OUTPUT" "spaces converted to dashes"

# Test 13: k8s_label_safe - special characters
create_template "k8s_label_special.tmpltool" '{{ k8s_label_safe(value="My App (v2.0)!") }}'
OUTPUT=$(run_binary "k8s_label_special.tmpltool")
assert_contains "$OUTPUT" "my-app" "special chars converted"

# Test 14: k8s_label_safe - underscores and dots
create_template "k8s_label_underscore_dot.tmpltool" '{{ k8s_label_safe(value="my_app.v1") }}'
OUTPUT=$(run_binary "k8s_label_underscore_dot.tmpltool")
assert_equals "my_app.v1" "$OUTPUT" "underscores and dots preserved"

# Test 15: k8s_label_safe - in labels
create_template "k8s_labels.tmpltool" '{% set app_name = "My Application" %}
{% set version = "v2.0.1" %}
metadata:
  labels:
    app: {{ k8s_label_safe(value=app_name) }}
    version: {{ k8s_label_safe(value=version) }}'
OUTPUT=$(run_binary "k8s_labels.tmpltool")
...
create_template "k8s_label_long.tmpltool" '{{ k8s_label_safe(value="this-is-a-very-long-label-name-that-exceeds-the-kubernetes-maximum-label-length-limit") }}'
OUTPUT=$(run_binary "k8s_label_long.tmpltool")
LENGTH=${#OUTPUT}
...
create_template "k8s_dns_simple.tmpltool" '{{ k8s_dns_label_safe(value="my-service") }}'
OUTPUT=$(run_binary "k8s_dns_simple.tmpltool")
assert_equals "my-service" "$OUTPUT" "simple DNS label unchanged"

# Test 18: k8s_dns_label_safe - uppercase
create_template "k8s_dns_uppercase.tmpltool" '{{ k8s_dns_label_safe(value="MyService") }}'
OUTPUT=$(run_binary "k8s_dns_uppercase.tmpltool")
assert_equals "myservice" "$OUTPUT" "uppercase converted to lowercase"

# Test 19: k8s_dns_label_safe - spaces
create_template "k8s_dns_spaces.tmpltool" '{{ k8s_dns_label_safe(value="My Service Name") }}'
OUTPUT=$(run_binary "k8s_dns_spaces.tmpltool")
assert_equals "my-service-name" "$OUTPUT" "spaces converted to dashes"

# Test 20: k8s_dns_label_safe - underscores removed
create_template "k8s_dns_underscore.tmpltool" '{{ k8s_dns_label_safe(value="my_service") }}'
OUTPUT=$(run_binary "k8s_dns_underscore.tmpltool")
assert_equals "my-service" "$OUTPUT" "underscores converted to dashes"

# Test 21: k8s_dns_label_safe - dots removed
create_template "k8s_dns_dots.tmpltool" '{{ k8s_dns_label_safe(value="my.service.v1") }}'
OUTPUT=$(run_binary "k8s_dns_dots.tmpltool")
assert_equals "my-service-v1" "$OUTPUT" "dots converted to dashes"

# Test 22: k8s_dns_label_safe - multiple dashes
create_template "k8s_dns_multiple_dashes.tmpltool" '{{ k8s_dns_label_safe(value="my---service") }}'
OUTPUT=$(run_binary "k8s_dns_multiple_dashes.tmpltool")
assert_equals "my-service" "$OUTPUT" "multiple dashes collapsed"

# Test 23: k8s_dns_label_safe - in service name
create_template "k8s_service.tmpltool" '{% set service_name = "My Service Name" %}
apiVersion: v1
kind: Service
metadata:
...
spec:
  selector:
    app: myapp'
OUTPUT=$(run_binary "k8s_service.tmpltool")
assert_contains "$OUTPUT" "name: my-service-name" "service name sanitized"

# Test 24: k8s_dns_label_safe - long string
create_template "k8s_dns_long.tmpltool" '{{ k8s_dns_label_safe(value="this-is-a-very-long-dns-label-that-exceeds-the-kubernetes-maximum-dns-label-length-limit") }}'
OUTPUT=$(run_binary "k8s_dns_long.tmpltool")
LENGTH=${#OUTPUT}
if [ $LENGTH -gt 63 ]; then
...
create_template "k8s_full_deployment.tmpltool" '{% set app_name = "My Application" %}
{% set version = "v2.0.1" %}
{% set cpu = "500m" %}
{% set memory = 512 %}
---
...
OUTPUT=$(run_binary "k8s_full_deployment.tmpltool")
...
create_template "k8s_multiple_services.tmpltool" '{% set services = [
...
{% endfor %}'
OUTPUT=$(run_binary "k8s_multiple_services.tmpltool")
...
create_template "k8s_env_resources.tmpltool" '{% set env = "production" %}
{% set app = "my-app" %}
...
OUTPUT=$(run_binary "k8s_env_resources.tmpltool")
...
create_template "error_k8s_missing_cpu.tmpltool" '{{ k8s_resource_request(memory="512Mi") }}'
OUTPUT=$(run_binary_expect_error "error_k8s_missing_cpu.tmpltool")
assert_contains "$OUTPUT" "error" "error on missing cpu"

# Test 29: Error - k8s_resource_request missing memory
create_template "error_k8s_missing_memory.tmpltool" '{{ k8s_resource_request(cpu="500m") }}'
OUTPUT=$(run_binary_expect_error "error_k8s_missing_memory.tmpltool")
