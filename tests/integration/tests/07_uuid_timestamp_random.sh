...
create_template "uuid.tmpltool" '{{ uuid() }}'
OUTPUT=$(run_binary "uuid.tmpltool")
...
create_template "uuid_v4.tmpltool" '{{ uuid(version="v4") }}'
OUTPUT=$(run_binary "uuid_v4.tmpltool")
...
create_template "uuid_v7.tmpltool" '{{ uuid(version="v7") }}'
OUTPUT=$(run_binary "uuid_v7.tmpltool")
...
create_template "uuid_invalid.tmpltool" '{{ uuid(version="v99") }}'
OUTPUT=$(run_binary "uuid_invalid.tmpltool" 2>&1) || true
...
create_template "timestamp.tmpltool" '{{ now() }}'
OUTPUT=$(run_binary "timestamp.tmpltool")
assert_matches "$OUTPUT" '^[0-9]+$' "now() returns valid Unix timestamp"

# Test 5b: now() with format returns ISO8601 timestamp
create_template "timestamp_formatted.tmpltool" '{{ now(format="%Y-%m-%dT%H:%M:%S") }}'
OUTPUT=$(run_binary "timestamp_formatted.tmpltool")
...
create_template "random.tmpltool" '{{ get_random(start=1, end=100) }}'
OUTPUT=$(run_binary "random.tmpltool")
