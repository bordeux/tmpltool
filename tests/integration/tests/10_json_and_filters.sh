...
create_template "json.tmpltool" "{% set data = parse_json(string='{\"name\": \"test\", \"value\": 42}') %}{{ data.name }}"
OUTPUT=$(run_binary "json.tmpltool")
assert_equals "test" "$OUTPUT" "JSON parsing works"

# Test 2: Filters work correctly
create_template "filter.tmpltool" '{{ "Hello World" | upper }}'
OUTPUT=$(run_binary "filter.tmpltool")
