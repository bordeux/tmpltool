...
create_template "conditional.tmpltool" '{% if get_env(name="ENABLE_FEATURE") == "true" %}enabled{% else %}disabled{% endif %}'
OUTPUT=$(ENABLE_FEATURE="true" run_binary "conditional.tmpltool")
assert_equals "enabled" "$OUTPUT" "Conditional evaluates to true"

# Test 2: Conditional evaluates to false
OUTPUT=$(ENABLE_FEATURE="false" run_binary "conditional.tmpltool")
assert_equals "disabled" "$OUTPUT" "Conditional evaluates to false"

# Test 3: Loop iteration
create_template "loop.tmpltool" '{% for i in [1, 2, 3] %}{{ i }}{% endfor %}'
OUTPUT=$(run_binary "loop.tmpltool")
