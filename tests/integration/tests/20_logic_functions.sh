...
create_template "default_truthy.tmpl" '{{ default(value="Hello", default="N/A") }}'
OUTPUT=$(run_binary "default_truthy.tmpl")
assert_equals "Hello" "$OUTPUT" "default returns truthy value"

# Test 2: default - empty string
create_template "default_empty_string.tmpl" '{{ default(value="", default="N/A") }}'
OUTPUT=$(run_binary "default_empty_string.tmpl")
assert_equals "N/A" "$OUTPUT" "default returns default for empty string"

# Test 3: default - with variables
create_template "default_variables.tmpl" '{% set config = {"port": 8080} %}
Host: {{ default(value=config.host, default="localhost") }}
Port: {{ default(value=config.port, default=3000) }}'
OUTPUT=$(run_binary "default_variables.tmpl")
...
create_template "default_false.tmpl" '{{ default(value=false, default="Default") }}'
OUTPUT=$(run_binary "default_false.tmpl")
assert_equals "Default" "$OUTPUT" "default treats false as falsy"

# Test 5: default - true value
create_template "default_true.tmpl" '{{ default(value=true, default="Default") }}'
OUTPUT=$(run_binary "default_true.tmpl")
assert_equals "true" "$OUTPUT" "default returns true value"

# Test 6: default - number value
create_template "default_number.tmpl" '{% set count = 42 %}
Count: {{ default(value=count, default=0) }}'
OUTPUT=$(run_binary "default_number.tmpl")
...
create_template "ternary_true.tmpl" '{{ ternary(condition=true, true_val="Yes", false_val="No") }}'
OUTPUT=$(run_binary "ternary_true.tmpl")
assert_equals "Yes" "$OUTPUT" "ternary returns true_val for true"

# Test 13: ternary - false condition
create_template "ternary_false.tmpl" '{{ ternary(condition=false, true_val="Yes", false_val="No") }}'
OUTPUT=$(run_binary "ternary_false.tmpl")
assert_equals "No" "$OUTPUT" "ternary returns false_val for false"

# Test 14: ternary - with comparison
create_template "ternary_comparison.tmpl" '{% set score = 85 %}
Result: {{ ternary(condition=score >= 60, true_val="Pass", false_val="Fail") }}'
OUTPUT=$(run_binary "ternary_comparison.tmpl")
assert_contains "$OUTPUT" "Result: Pass" "ternary works with comparison"

# Test 15: ternary - status indicator
create_template "ternary_status.tmpl" '{% set cpu_usage = 75 %}
Status: {{ ternary(
  condition=cpu_usage > 90,
  true_val="Critical",
  false_val="Normal"
) }}'
OUTPUT=$(run_binary "ternary_status.tmpl")
assert_contains "$OUTPUT" "Status: Normal" "ternary evaluates condition"

# Test 16: ternary - with numbers
create_template "ternary_numbers.tmpl" '{% set enabled = true %}
Max connections: {{ ternary(condition=enabled, true_val=100, false_val=10) }}'
OUTPUT=$(run_binary "ternary_numbers.tmpl")
assert_contains "$OUTPUT" "Max connections: 100" "ternary works with numeric values"

# Test 17: ternary - truthy string
create_template "ternary_truthy_string.tmpl" '{{ ternary(condition="hello", true_val="Yes", false_val="No") }}'
OUTPUT=$(run_binary "ternary_truthy_string.tmpl")
assert_equals "Yes" "$OUTPUT" "ternary treats non-empty string as truthy"

# Test 18: ternary - empty string
create_template "ternary_empty_string.tmpl" '{{ ternary(condition="", true_val="Yes", false_val="No") }}'
OUTPUT=$(run_binary "ternary_empty_string.tmpl")
...
OUTPUT=$(run_binary "config_all_functions.tmpl")
...
create_template "resource_limits.tmpl" '{% set cpu = 75 %}
{% set memory = 85 %}
{% set disk = 95 %}
...
OUTPUT=$(run_binary "resource_limits.tmpl")
...
create_template "fallback_chain.tmpl" '{% set primary = none %}
{% set secondary = none %}
...
Max Connections: {{ max_conn }}
Log Level: {{ log_level }}'
OUTPUT=$(run_binary "env_based_config.tmpl")
...
create_template "error_coalesce_non_array.tmpl" '{{ coalesce(values="test") }}'
OUTPUT=$(run_binary_expect_error "error_coalesce_non_array.tmpl")
assert_contains "$OUTPUT" "error" "coalesce rejects non-array"

# Test 36: Error - in_range with non-numeric value
create_template "error_in_range_non_numeric.tmpl" '{{ in_range(value="test", min=0, max=100) }}'
OUTPUT=$(run_binary_expect_error "error_in_range_non_numeric.tmpl")
assert_contains "$OUTPUT" "error" "in_range rejects non-numeric value"

# Test 37: Error - in_range with non-numeric min
create_template "error_in_range_non_numeric_min.tmpl" '{{ in_range(value=50, min="test", max=100) }}'
OUTPUT=$(run_binary_expect_error "error_in_range_non_numeric_min.tmpl")
