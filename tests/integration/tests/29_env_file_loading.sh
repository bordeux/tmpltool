...
ANOTHER_VAR=world
EOF
create_template "env_file.tmpltool" '{{ get_env(name="TEST_VAR") }} {{ get_env(name="ANOTHER_VAR") }}'
OUTPUT=$("$BINARY" --env "$TEST_DIR/.env" "$TEST_DIR/env_file.tmpltool" 2>&1)
...
VAR3=new_value
EOF
create_template "multi_env.tmpltool" '{{ get_env(name="VAR1") }}-{{ get_env(name="VAR2") }}-{{ get_env(name="VAR3") }}'
OUTPUT=$("$BINARY" --env "$TEST_DIR/base.env" --env "$TEST_DIR/override.env" "$TEST_DIR/multi_env.tmpltool" 2>&1)
...
# Another comment
EOF
create_template "comments.tmpltool" '{{ get_env(name="VAR") }}'
OUTPUT=$("$BINARY" --env "$TEST_DIR/comments.env" "$TEST_DIR/comments.tmpltool" 2>&1)
...
EOF
create_template "quoted.tmpltool" '{{ get_env(name="DOUBLE") }}|{{ get_env(name="SINGLE") }}'
OUTPUT=$("$BINARY" --env "$TEST_DIR/quoted.env" "$TEST_DIR/quoted.tmpltool" 2>&1)
...
EMPTY=
EOF
create_template "empty.tmpltool" '[{{ get_env(name="EMPTY") }}]'
OUTPUT=$("$BINARY" --env "$TEST_DIR/empty.env" "$TEST_DIR/empty.tmpltool" 2>&1)
...
EOF
create_template "special.tmpltool" '{{ get_env(name="URL") }}'
OUTPUT=$("$BINARY" --env "$TEST_DIR/special.env" "$TEST_DIR/special.tmpltool" 2>&1)
...
create_template "missing_env.tmpltool" '{{ get_env(name="X", default="default") }}'
set +e
OUTPUT=$("$BINARY" --env "$TEST_DIR/nonexistent.env" "$TEST_DIR/missing_env.tmpltool" 2>&1)
EXIT_CODE=$?
set -e
...
EOF
create_template "output.tmpltool" '{{ get_env(name="OUTPUT_VAR") }}'
"$BINARY" --env "$TEST_DIR/output.env" -o "$TEST_DIR/output.txt" "$TEST_DIR/output.tmpltool" 2>&1
...
PORT=8080
EOF
create_template "validate.tmpltool" '{"app": "{{ get_env(name="APP_NAME") }}", "port": {{ get_env(name="PORT") }}}'
OUTPUT=$("$BINARY" --env "$TEST_DIR/validate.env" --validate json "$TEST_DIR/validate.tmpltool" 2>&1)
...
VALUE=third
EOF
create_template "order.tmpltool" '{{ get_env(name="VALUE") }}'
OUTPUT=$("$BINARY" --env "$TEST_DIR/first.env" --env "$TEST_DIR/second.env" --env "$TEST_DIR/third.env" "$TEST_DIR/order.tmpltool" 2>&1)
...
TRUST_VAR=trusted
EOF
create_template "trust.tmpltool" '{{ get_env(name="TRUST_VAR") }}'
OUTPUT=$("$BINARY" --trust --env "$TEST_DIR/trust.env" "$TEST_DIR/trust.tmpltool" 2>&1)
...
line3"
EOF
create_template "multiline.tmpltool" '{{ get_env(name="MULTI") }}'
OUTPUT=$("$BINARY" --env "$TEST_DIR/multiline.env" "$TEST_DIR/multiline.tmpltool" 2>&1)
...
EOF
create_template "nopollute.tmpltool" '{{ get_env(name="UNIQUE_VAR_12345") }}'
"$BINARY" --env "$TEST_DIR/nopollute.env" "$TEST_DIR/nopollute.tmpltool" > /dev/null 2>&1
