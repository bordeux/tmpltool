...
create_template "helm1.tmpl" '{{ helm_tpl(template="Hello {{ .name }}!", values={"name": "World"}) }}'
OUTPUT=$(run_binary "helm1.tmpl")
assert_equals "Hello World!" "$OUTPUT" "helm_tpl() simple substitution"

# Test: helm_tpl() - nested values
create_template "helm2.tmpl" '{{ helm_tpl(template="{{ .app.name }}-{{ .app.version }}", values={"app": {"name": "myapp", "version": "1.0"}}) }}'
OUTPUT=$(run_binary "helm2.tmpl")
assert_equals "myapp-1.0" "$OUTPUT" "helm_tpl() nested values"

# Test: helm_tpl() - no placeholders
create_template "helm3.tmpl" '{{ helm_tpl(template="static-text", values={}) }}'
OUTPUT=$(run_binary "helm3.tmpl")
assert_equals "static-text" "$OUTPUT" "helm_tpl() no placeholders"

# ==================== k8s_annotation_safe Tests ====================

# Test: k8s_annotation_safe() - simple text
create_template "anno1.tmpl" '{{ k8s_annotation_safe(value="simple annotation") }}'
OUTPUT=$(run_binary "anno1.tmpl")
...
create_template "anno2.tmpl" '{{ k8s_annotation_safe(value="line1 line2") }}'
OUTPUT=$(run_binary "anno2.tmpl")
...
create_template "qty2b1.tmpl" '{{ k8s_quantity_to_bytes(quantity="1Gi") }}'
OUTPUT=$(run_binary "qty2b1.tmpl")
assert_equals "1073741824" "$OUTPUT" "k8s_quantity_to_bytes() 1Gi"

# Test: k8s_quantity_to_bytes() - Mi
create_template "qty2b2.tmpl" '{{ k8s_quantity_to_bytes(quantity="512Mi") }}'
OUTPUT=$(run_binary "qty2b2.tmpl")
assert_equals "536870912" "$OUTPUT" "k8s_quantity_to_bytes() 512Mi"

# Test: k8s_quantity_to_bytes() - Ki
create_template "qty2b3.tmpl" '{{ k8s_quantity_to_bytes(quantity="100Ki") }}'
OUTPUT=$(run_binary "qty2b3.tmpl")
assert_equals "102400" "$OUTPUT" "k8s_quantity_to_bytes() 100Ki"

# Test: k8s_quantity_to_bytes() - decimal G
create_template "qty2b4.tmpl" '{{ k8s_quantity_to_bytes(quantity="1G") }}'
OUTPUT=$(run_binary "qty2b4.tmpl")
assert_equals "1000000000" "$OUTPUT" "k8s_quantity_to_bytes() 1G"

# Test: k8s_quantity_to_bytes() - millicores
create_template "qty2b5.tmpl" '{{ k8s_quantity_to_bytes(quantity="500m") }}'
OUTPUT=$(run_binary "qty2b5.tmpl")
assert_equals "500" "$OUTPUT" "k8s_quantity_to_bytes() 500m (millicores)"

# ==================== k8s_bytes_to_quantity Tests ====================

# Test: k8s_bytes_to_quantity() - 1Gi
create_template "b2qty1.tmpl" '{{ k8s_bytes_to_quantity(bytes=1073741824) }}'
OUTPUT=$(run_binary "b2qty1.tmpl")
assert_equals "1Gi" "$OUTPUT" "k8s_bytes_to_quantity() 1Gi"

# Test: k8s_bytes_to_quantity() - 512Mi
create_template "b2qty2.tmpl" '{{ k8s_bytes_to_quantity(bytes=536870912) }}'
OUTPUT=$(run_binary "b2qty2.tmpl")
assert_equals "512Mi" "$OUTPUT" "k8s_bytes_to_quantity() 512Mi"

# Test: k8s_bytes_to_quantity() - forced unit
create_template "b2qty3.tmpl" '{{ k8s_bytes_to_quantity(bytes=1073741824, unit="Mi") }}'
OUTPUT=$(run_binary "b2qty3.tmpl")
assert_equals "1024Mi" "$OUTPUT" "k8s_bytes_to_quantity() forced Mi"

# Test: k8s_bytes_to_quantity() - zero
create_template "b2qty4.tmpl" '{{ k8s_bytes_to_quantity(bytes=0) }}'
OUTPUT=$(run_binary "b2qty4.tmpl")
assert_equals "0" "$OUTPUT" "k8s_bytes_to_quantity() zero"

# ==================== k8s_selector Tests ====================

# Test: k8s_selector() - single label
create_template "sel1.tmpl" '{{ k8s_selector(labels={"app": "nginx"}) }}'
OUTPUT=$(run_binary "sel1.tmpl")
assert_equals "app=nginx" "$OUTPUT" "k8s_selector() single label"

# Test: k8s_selector() - multiple labels
create_template "sel2.tmpl" '{{ k8s_selector(labels={"app": "nginx", "env": "prod"}) }}'
OUTPUT=$(run_binary "sel2.tmpl")
...
create_template "sel3.tmpl" '{{ k8s_selector(labels={}) }}'
OUTPUT=$(run_binary "sel3.tmpl")
assert_equals "" "$OUTPUT" "k8s_selector() empty"

# ==================== k8s_pod_affinity Tests ====================

# Test: k8s_pod_affinity() - required
create_template "aff1.tmpl" '{{ k8s_pod_affinity(key="app", operator="In", values=["web", "api"], type="required") }}'
OUTPUT=$(run_binary "aff1.tmpl")
...
create_template "aff2.tmpl" '{{ k8s_pod_affinity(key="app", operator="In", values=["web"], type="preferred") }}'
OUTPUT=$(run_binary "aff2.tmpl")
...
create_template "aff3.tmpl" '{{ k8s_pod_affinity(key="app", operator="Exists") }}'
OUTPUT=$(run_binary "aff3.tmpl")
assert_contains "$OUTPUT" "operator: Exists" "k8s_pod_affinity() Exists"

# ==================== k8s_toleration Tests ====================

# Test: k8s_toleration() - Equal operator
create_template "tol1.tmpl" '{{ k8s_toleration(key="dedicated", value="gpu", effect="NoSchedule") }}'
OUTPUT=$(run_binary "tol1.tmpl")
...
create_template "tol2.tmpl" '{{ k8s_toleration(key="node.kubernetes.io/not-ready", operator="Exists", effect="NoExecute") }}'
OUTPUT=$(run_binary "tol2.tmpl")
...
create_template "probe1.tmpl" '{{ k8s_probe(type="http", path="/health", port=8080) }}'
OUTPUT=$(run_binary "probe1.tmpl")
...
create_template "probe2.tmpl" '{{ k8s_probe(type="tcp", port=5432) }}'
OUTPUT=$(run_binary "probe2.tmpl")
...
create_template "probe3.tmpl" '{{ k8s_probe(type="exec", command=["cat", "/tmp/healthy"]) }}'
OUTPUT=$(run_binary "probe3.tmpl")
...
create_template "probe4.tmpl" '{{ k8s_probe(type="http", path="/healthz", port=8080, initial_delay=15, period=20, failure_threshold=5) }}'
OUTPUT=$(run_binary "probe4.tmpl")
...
create_template "k8s_deploy.tmpl" 'spec:
  containers:
    - name: app
      resources:
...
OUTPUT=$(run_binary "k8s_deploy.tmpl")
...
create_template "qty_roundtrip.tmpl" '{% set bytes = k8s_quantity_to_bytes(quantity="2Gi") %}{{ k8s_bytes_to_quantity(bytes=bytes) }}'
OUTPUT=$(run_binary "qty_roundtrip.tmpl")
