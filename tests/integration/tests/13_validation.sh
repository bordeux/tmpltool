...
create_template "valid_json.tmpltool" '{"valid": "json"}'
set +e
"$BINARY" "$TEST_DIR/valid_json.tmpltool" --validate json >/dev/null 2>&1
EXIT_CODE=$?
set -e
assert_exit_code 0 "$EXIT_CODE" "Valid JSON passes validation"

# Test 2: Invalid JSON fails validation
create_template "invalid_json.tmpltool" '{invalid json}'
set +e
"$BINARY" "$TEST_DIR/invalid_json.tmpltool" --validate json >/dev/null 2>&1
EXIT_CODE=$?
set -e
