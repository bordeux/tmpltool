...
create_template "array_count_basic.tmpltool" '{% set items = ["apple", "banana", "cherry"] %}{{ array_count(array=items) }}'
OUTPUT=$(run_binary "array_count_basic.tmpltool")
assert_equals "3" "$OUTPUT" "array_count returns correct count"

# Test 2: array_count - empty array
create_template "array_count_empty.tmpltool" '{% set items = [] %}{{ array_count(array=items) }}'
OUTPUT=$(run_binary "array_count_empty.tmpltool")
assert_equals "0" "$OUTPUT" "array_count returns 0 for empty array"

# Test 3: array_count - single element
create_template "array_count_single.tmpltool" '{% set items = [42] %}{{ array_count(array=items) }}'
OUTPUT=$(run_binary "array_count_single.tmpltool")
...
create_template "array_chunk_even.tmpltool" '{% set nums = [1, 2, 3, 4, 5, 6] %}
{% for chunk in array_chunk(array=nums, size=2) %}
{{ chunk }}
{% endfor %}'
OUTPUT=$(run_binary "array_chunk_even.tmpltool")
...
create_template "array_chunk_uneven.tmpltool" '{% set nums = [1, 2, 3, 4, 5] %}
{% for chunk in array_chunk(array=nums, size=2) %}
{{ chunk }}
{% endfor %}'
OUTPUT=$(run_binary "array_chunk_uneven.tmpltool")
...
create_template "array_chunk_size_one.tmpltool" '{% set nums = [1, 2, 3] -%}
{{- array_chunk(array=nums, size=1) | length -}}'
OUTPUT=$(run_binary "array_chunk_size_one.tmpltool")
assert_equals "3" "$OUTPUT" "array_chunk with size 1 creates individual chunks"

# Test 7: array_chunk - larger than array
create_template "array_chunk_large.tmpltool" '{% set nums = [1, 2, 3] -%}
{{- array_chunk(array=nums, size=10) | length -}}'
OUTPUT=$(run_binary "array_chunk_large.tmpltool")
...
create_template "array_zip_equal.tmpltool" '{% set keys = ["name", "age", "city"] %}
{% set values = ["Alice", 30, "NYC"] %}
{% for pair in array_zip(array1=keys, array2=values) %}
{{ pair[0] }}: {{ pair[1] }}
{% endfor %}'
OUTPUT=$(run_binary "array_zip_equal.tmpltool")
...
create_template "array_zip_different.tmpltool" '{% set a = [1, 2, 3, 4] -%}
{%- set b = ["a", "b"] -%}
{{- array_zip(array1=a, array2=b) | length -}}'
OUTPUT=$(run_binary "array_zip_different.tmpltool")
assert_equals "2" "$OUTPUT" "array_zip stops at shorter array length"

# Test 10: array_zip - empty arrays
create_template "array_zip_empty.tmpltool" '{% set a = [] -%}
{%- set b = [] -%}
{{- array_zip(array1=a, array2=b) | length -}}'
OUTPUT=$(run_binary "array_zip_empty.tmpltool")
assert_equals "0" "$OUTPUT" "array_zip handles empty arrays"

# Test 11: array_zip - first empty
create_template "array_zip_first_empty.tmpltool" '{% set a = [] -%}
{%- set b = [1, 2, 3] -%}
{{- array_zip(array1=a, array2=b) | length -}}'
OUTPUT=$(run_binary "array_zip_first_empty.tmpltool")
...
create_template "pagination.tmpltool" '{% set items = ["a", "b", "c", "d", "e", "f"] %}
{% for page in array_chunk(array=items, size=3) %}
Page: {{ page | join(", ") }}
{% endfor %}'
OUTPUT=$(run_binary "pagination.tmpltool")
...
create_template "key_value_map.tmpltool" '{% set k = ["host", "port", "user"] %}
{% set v = ["localhost", 8080, "admin"] %}
{% for pair in array_zip(array1=k, array2=v) %}
{{ pair[0] }}={{ pair[1] }}
{% endfor %}'
OUTPUT=$(run_binary "key_value_map.tmpltool")
...
create_template "count_conditional.tmpltool" '{% set items = [1, 2, 3, 4, 5] %}
