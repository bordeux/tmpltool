...
// --- object_flatten error and edge cases ---

#[test]
fn test_object_flatten_not_object() {
    let arr = serde_json::json!([1, 2, 3]);
...
#[test]
fn test_object_flatten_with_array() {
    let obj = serde_json::json!({"data": {"items": [1, 2, 3]}});
...
#[test]
fn test_object_flatten_null_values() {
    let obj = serde_json::json!({"a": null, "b": {"c": null}});
    ...
    assert!(json["b.c"].is_null());
}

#[test]
fn test_object_flatten_deep_array() {
    let obj = serde_json::json!({"matrix": [[1, 2], [3, 4]]});
    ...
    let json: serde_json::Value = serde_json::to_value(&result).unwrap();
    assert_eq!(json["matrix.0.0"], 1);
    assert_eq!(json["matrix.0.1"], 2);
    assert_eq!(json["matrix.1.0"], 3);
    assert_eq!(json["matrix.1.1"], 4);
}
...
fn test_object_get_missing_object() {
    let result = object::object_get_fn(Kwargs::from_iter(vec![("path", Value::from("a.b"))]));
...
#[test]
fn test_object_keys_missing_object() {
    let result = object::object_keys_fn(Kwargs::from_iter(Vec::<(&str, Value)>::new()));

    assert!(result.is_err());
}

#[test]
fn test_object_values_missing_object() {
    let result = object::object_values_fn(Kwargs::from_iter(Vec::<(&str, Value)>::new()));
...
fn test_object_has_key_missing_object() {
    let result = object::object_has_key_fn(Kwargs::from_iter(vec![("key", Value::from("test"))]));
...
fn test_json_path_missing_object() {
    let result = object::json_path_fn(Kwargs::from_iter(vec![("path", Value::from("$.name"))]));
...
#[test]
fn test_object_flatten_missing_object() {
    let result = object::object_flatten_fn(Kwargs::from_iter(Vec::<(&str, Value)>::new()));
...
fn test_object_unflatten_missing_object() {
    let result = object::object_unflatten_fn(Kwargs::from_iter(Vec::<(&str, Value)>::new()));
