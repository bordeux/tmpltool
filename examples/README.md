...
```bash
tmpltool examples/<template-name>.tmpl -o output.txt
```

Or print to stdout:

```bash
tmpltool examples/<template-name>.tmpl
```
...
# With custom values
NAME="Alice" GREETING_TIME="morning" tmpltool examples/greeting.tmpl
```
...
APP_ENV=development \
APP_VERSION=1.0.0 \
tmpltool examples/config.tmpl -o app.conf
```
...
LOG_LEVEL=debug \
DEBUG=true \
tmpltool examples/config-with-defaults.tmpl -o app.conf
```
