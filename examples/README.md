...
**Usage:**
```bash
CUSTOM_VAR="Hello World" tmpltool examples/basic.tmpltool
```
...
```bash
# Run with defaults
tmpltool examples/greeting.tmpltool

# With custom values
NAME="Alice" GREETING_TIME="morning" tmpltool examples/greeting.tmpltool
```
...
APP_ENV=development \
APP_VERSION=1.0.0 \
tmpltool examples/config.tmpltool -o app.conf
```
...
tmpltool examples/docker-compose.tmpltool -o docker-compose.yml
...
### 5. config-with-defaults.tmpltool
...
tmpltool examples/config-with-defaults.tmpltool
...
LOG_LEVEL=debug \
DEBUG=true \
tmpltool examples/config-with-defaults.tmpltool -o app.conf
```
